...
namespace Game.Hotfix
{
    ...
    internal sealed partial class ObjectPoolManager : IObjectPoolManager
    {
        ...
        {
            return ContainsPoolName(Utility.Text.GetFullName(typeof(T), string.Empty));
        }
        ...
        public bool HasObjectPool(Type objectType)
        {
            ...
            return ContainsPoolName(Utility.Text.GetFullName(objectType, string.Empty));
        }
        ...
        {
            return ContainsPoolName(Utility.Text.GetFullName(typeof(T), name));
        }
        ...
        public bool HasObjectPool(Type objectType, string name)
        {
            ...
            return ContainsPoolName(Utility.Text.GetFullName(objectType, name));
        }
        ...
        /// <returns>是否存在对象池。</returns>
        public bool ContainsPoolName(string fullName)
        {
        ...
        {
            return (IObjectPool<T>)GetObjectPool(Utility.Text.GetFullName(typeof(T), string.Empty));
        }
        ...
        {
            return (IObjectPool<T>)GetObjectPool(Utility.Text.GetFullName(typeof(T), name));
        }
        ...
        {
            return InternalDestroyObjectPool(Utility.Text.GetFullName(typeof(T), string.Empty));
        }
        ...
        {
            return InternalDestroyObjectPool(Utility.Text.GetFullName(typeof(T), name));
        }
        ...
        public bool DestroyObjectPool<T>(IObjectPool<T> objectPool) where T : ObjectBase
        {
            ...
            return InternalDestroyObjectPool(Utility.Text.GetFullName(typeof(T), objectPool.Name));
        }
        ...
        private IObjectPool<T> InternalCreateObjectPool<T>(string name, bool allowMultiSpawn, float autoReleaseInterval, int capacity, float expireTime, int priority) where T : ObjectBase
        {
            ...
            {
                throw new Exception(Utility.Text.Format("Already exist object pool '{0}'.", Utility.Text.GetFullName(typeof(T), name)));
            }

            ObjectPool<T> objectPool = new ObjectPool<T>(name, allowMultiSpawn, autoReleaseInterval, capacity, expireTime, priority);
            m_ObjectPools.Add(Utility.Text.GetFullName(typeof(T), name), objectPool);
            return objectPool;
        }
        ...
        private ObjectPoolBase InternalCreateObjectPool(Type objectType, string name, bool allowMultiSpawn, float autoReleaseInterval, int capacity, float expireTime, int priority)
        {
            ...
            {
                throw new Exception(Utility.Text.Format("Already exist object pool '{0}'.", Utility.Text.GetFullName(objectType, name)));
            }
