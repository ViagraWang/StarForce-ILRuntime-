...
namespace UnityGameFrame.Editor.AssetBundleTools
{
    public sealed partial class AssetBundleBuilderController
    {
        ...
        public AssetBundleBuilderController()
        {
            ...
            m_AssetBundleAnalyzerController.EventOnAnalyzingAsset += delegate (int index, int count)
            {
            ...
            m_AssetBundleAnalyzerController.EventOnAnalyzeCompleted += delegate ()
            {
            ...
            m_BuildEventHandlerTypeNames.AddRange(Type.GetEditorSubClassNames(typeof(IBuildEventHandler)));
        ...
        private bool BuildAssetBundles(Platform platform, AssetBundleBuild[] buildMap, BuildAssetBundleOptions buildOptions)
        {
            ...
            if (OutputFullSelected)
            {
                ...
                if (m_BuildEventHandler != null)
                    m_BuildEventHandler.OnHandleVersionList(outputFullPath, ApplicableGameVersion, InternalResourceVersion, versionListData.Length, versionListData.HashCode, versionListData.ZipLength, versionListData.ZipHashCode);
            }
        ...
        private void ProcessPackageList(string outputPackagePath, Platform platform)
        {
            ...
            //string packageListFilePath = Utility.Path.GetCombinePath(outputPackagePath, VersionListFileName + ".txt");
            //FileStream fs = new FileStream(packageListFilePath, FileMode.CreateNew, FileAccess.Write);
            ...
            using (FileStream fileStream = new FileStream(packageListPath, FileMode.CreateNew, FileAccess.Write))
            {
                using (BinaryWriter binaryWriter = new BinaryWriter(fileStream, Encoding.UTF8))
                {
                    ...
                    //sw.WriteLine("PackageListHeader:" + PackageListHeader);
                    binaryWriter.Write(PackageListVersion);
                    //sw.WriteLine("PackageListVersion:" + PackageListVersion);
                    ...
                    //sw.WriteLine("ApplicableGameVersion:" + ApplicableGameVersion);
                    binaryWriter.Write(InternalResourceVersion);
                    //sw.WriteLine("InternalResourceVersion:" + InternalResourceVersion);
                    binaryWriter.Write(m_AssetBundleCollection.AssetCount);
                    //sw.WriteLine("AssetCount:" + m_AssetBundleCollection.AssetCount);
                    binaryWriter.Write(m_AssetBundleCollection.AssetBundleCount);
                    //sw.WriteLine("AssetBundleCount:" + m_AssetBundleCollection.AssetBundleCount);
                    if (m_AssetBundleCollection.AssetBundleCount > ushort.MaxValue) //超过数量限制
                    {
                        throw new GameFrameworkException("[AssetBundleBuilderController.ProcessPackageList] Package list can only contains 65535 resources in version 0.");
                    }
                    ...
                    foreach (AssetBundleData assetBundleData in m_AssetBundleDatas.Values)
                    {
                        ...
                        //sw.Write("AssetBundleName:" + assetBundleData.Name);
                        ...
                        foreach (string assetName in assetNames)
                        {
                            ...
                            //sw.Write("(dependencyAssetNames.Length:" + dependencyAssetNames.Length);
                    ...
                    //资源组处理
                    string[] resourceGroups = GetResourceGroups();  //获取资源组
                    binaryWriter.Write(resourceGroups.Length);  //写入资源组数量
                    for (int i = 0; i < resourceGroups.Length; i++)
                    {
                        string resourceGroup = resourceGroups[i];
                        byte[] resourceGroupNameBytes = GetXorBytes(Utility.Converter.GetBytes(resourceGroup), encryptBytes);
                        if(resourceGroupNameBytes.Length > byte.MaxValue)   //超出上限
                        ...
                        for (int j = 0; j < resourceIndices.Length; j++)
                        {
                            binaryWriter.Write(resourceIndices[j]);
                        }
                    }
        ...
        private VersionListData ProcessVersionList(string outputFullPath, Platform platform)
        {
            ...
            //string versionListFilePath = Utility.Path.GetCombinePath(outputFullPath, VersionListFileName + ".txt");
            //FileStream fs = new FileStream(versionListFilePath, FileMode.CreateNew, FileAccess.Write);
            //StreamWriter sw = new StreamWriter(fs);
            ...
            using (FileStream fileStream = new FileStream(versionListPath, FileMode.CreateNew, FileAccess.Write))
            {
                using (BinaryWriter binaryWriter = new BinaryWriter(fileStream, Encoding.UTF8))
                {
                    ...
                    //sw.WriteLine("VersionListHeader:" + VersionListHeader);
                    binaryWriter.Write(VersionListVersion);
                    //sw.WriteLine("VersionList:" + VersionListVersion);
                    binaryWriter.Write(encryptBytes);
                    ...
                    foreach (AssetBundleData assetBundleData in m_AssetBundleDatas.Values)
                    {
                        ...
                        //sw.Write("\tAssetBundle.ZipLength:" + assetBundleCode.ZipLength);
                        binaryWriter.Write(assetBundleCode.ZipHashCode);
                        //sw.Write("\tAssetBundle.ZipHashCode:" + assetBundleCode.ZipHashCode);
        ...
        private void ProcessReadOnlyList(string outputPackedPath, Platform platform)
        {
            ...
            //string readOnlyListFilePath = Utility.Path.GetCombinePath(outputPackedPath, ResourceListFileName + ".txt");
            //FileStream fs = new FileStream(readOnlyListFilePath, FileMode.CreateNew, FileAccess.Write);
            //StreamWriter sw = new StreamWriter(fs);
            ...
            using (FileStream fileStream = new FileStream(readOnlyListPath, FileMode.CreateNew, FileAccess.Write))
            {
                using (BinaryWriter binaryWriter = new BinaryWriter(fileStream, Encoding.UTF8))
                {
                    ...
                    //sw.WriteLine("ReadOnlyListHeader:" + ReadOnlyListHeader);
                    binaryWriter.Write(ReadOnlyListVersion);
                    //sw.WriteLine("ReadOnlyListVersion:" + ReadOnlyListVersion);
                    binaryWriter.Write(encryptBytes);

                    binaryWriter.Write(packedAssetBundleDatas.Count);
                    //sw.WriteLine("PackedAssetBundle.Count:" + packedAssetBundleDatas.Count);
                    ...
                    foreach (AssetBundleData assetBundleData in packedAssetBundleDatas)
                    {
                        ...
                        else
                        {
                            ...
                            binaryWriter.Write((byte)variantBytes.Length);
                            binaryWriter.Write(variantBytes);
                            //sw.WriteLine("\tVariant:" + assetBundleData.Variant);
                        }

                        binaryWriter.Write((byte)assetBundleData.LoadType);
                        //sw.WriteLine("\tLoadType:" + (byte)assetBundleData.LoadType);
                        AssetBundleCode assetBundleCode = assetBundleData.GetCode(platform);
                        binaryWriter.Write(assetBundleCode.Length);
                        //sw.WriteLine("\tAssetBundle.Length:" + assetBundleCode.Length);
                        binaryWriter.Write(assetBundleCode.HashCode);
                        //sw.WriteLine("\tAssetBundle.HashCode:" + assetBundleCode.HashCode);
                    }
                }
            }
            //sw.Close();
        ...
        private string[] GetResourceGroups()
        {
            ...
            foreach (AssetBundleData assetBundleData in m_AssetBundleDatas.Values)
            {
                string[] bundleResourceGroups = assetBundleData.GetResourceGroups();
                for (int i = 0; i < bundleResourceGroups.Length; i++)
                {
                    string resourceGroup = bundleResourceGroups[i];
        ...
        private ushort[] GetResourceIndices(string resourceGroup)
        {
            ...
            {
                string[] bundleResourceGroups = assetBundleDatas[i].GetResourceGroups();
                for (int j = 0; j < bundleResourceGroups.Length; j++)
                {
                    if (bundleResourceGroups[j].Equals(resourceGroup))
                    {
        ...
        private AssetBundleBuild[] GetBuildMap()
        {
            ...
            AssetBundleInfo[] assetBundleInfos = m_AssetBundleCollection.AllAssetBundleInfos;
            foreach (AssetBundleInfo assetBundleInfo in assetBundleInfos)
            {
                m_AssetBundleDatas.Add(assetBundleInfo.FullName.ToLower(), new AssetBundleData(assetBundleInfo.Name.ToLower(), assetBundleInfo.Variant != null ? assetBundleInfo.Variant.ToLower() : null, assetBundleInfo.LoadType, assetBundleInfo.Packed, assetBundleInfo.GetResourceGroups()));
            }

            AssetInfo[] assets = m_AssetBundleCollection.AllAssetInfos;
            foreach (AssetInfo asset in assets)
            {
                ...
                AssetInfo[] dependencyAssets = dependencyData.AllDependencyAssetInfos;
                foreach (AssetInfo dependencyAsset in dependencyAssets)
                {
                ...
                m_AssetBundleDatas[asset.AssetBundleInfo.FullName.ToLower()].AddAssetData(asset.Guid, assetName, assetBytes.Length, assetHashCode, dependencyAssetNames.ToArray());
            }
